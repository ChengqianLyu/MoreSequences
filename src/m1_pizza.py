...
def run_test_generate_points_on_circle():
    ...
    # ------------------------------------------------------------------
    expected = [rg.Point(25.0, 5.0),  # All numbers are approximate.
                rg.Point(17.5, 18.0),
                rg.Point(2.5, 18.0),
                rg.Point(-5.0, 5.0),
                rg.Point(2.5, -8.0),
                rg.Point(17.5, -8.0)]
    circle = rg.Circle(rg.Point(10, 5),
                       15)
    answer = generate_points_on_circle(circle, 6)
...
def run_test_draw_points_on_circle():
    ...
    # ------------------------------------------------------------------
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  100 purple dots.'
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(generate_points_on_circle(circle,number_of_points))):
        a = rg.Circle(generate_points_on_circle(circle,number_of_points)[k],10)
        a.fill_color = color
        a.attach_to(window)
        window.render()
    window.render()
...
def run_test_pizza():
    ...
    # ------------------------------------------------------------------
    title = 'PIZZA test 4:  100 slices, thin (thickness=2) black lines.'
    window = rg.RoseWindow(500, 500, title)
    circle = rg.Circle(rg.Point(200, 200), 200)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(len(generate_points_on_circle(circle,number_of_slices))):
        a = rg.Line(generate_points_on_circle(circle,number_of_slices)[k],circle.center)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    for k in range(len(generate_points_on_circle(circle,number_of_segments))):
        a = rg.Line(generate_points_on_circle(circle,number_of_segments)[k],generate_points_on_circle(circle,number_of_segments)[k-1])
        a.color = color
        a.thickness = thickness
        a.attach_to(window)
        window.render()
...
def run_test_fancy_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'FANCY POLYGON test 4:  30 black lines' + \
            ' on yellow circle, hops = 10.'
    window = rg.RoseWindow(500, 500, title)

    circle = rg.Circle(rg.Point(250, 250), 100)
    circle.fill_color = 'yellow'
    fancy_polygon(window, circle, 30, 10, 'black', 5)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines,
    ...
    for k in range(len(generate_points_on_circle(circle,number_of_lines))):
        a = rg.Line(generate_points_on_circle(circle,number_of_lines)[k],generate_points_on_circle(circle,number_of_lines)[k-hops_to_next_point])
        a.thickness = thickness
        a.color = color
        a.arrow = 'first'
        a.attach_to(window)
